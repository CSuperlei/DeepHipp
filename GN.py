...
class GroupNormalization(Layer):
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        # transpose:[ba,h,w,d,c] -> [bs,c,h,w,d]
        if self.axis in {-1, 4}:
            inputs = K.permute_dimensions(inputs, (0, 4, 1, 2, 3))

        input_shape = K.int_shape(inputs)
        N, C, H, W, D = input_shape
        inputs = K.reshape(inputs, (-1, G, C // G, H, W, D))
        # inputs.assign_sub()
        ...
        outputs = (inputs - gn_mean) / (K.sqrt(gn_variance + self.epsilon))
        outputs = K.reshape(outputs, [-1, C, H, W, D]) * self.gamma + self.beta

        # transpose: [bs,c,h,w,d] -> [ba,h,w,d,c]
        if self.axis in {-1, 4}:
